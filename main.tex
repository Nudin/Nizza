...
\end{equation}
Setzt man dies in den Hamiltonian \ref{eq:nbodyhamilton} ein, so erhält man nach einigen Umformungen: %durchrechnen, vorrechnen?
\begin{equation}
...
Jupiter und Saturn haben deutlich zu viele kleine Monde und die SFD des Modells (gleich der des Kuipergürtels) ist allgemein für Mondradien von $D\gtrsim \unit[10]{km}$ deutlich steiler als in Realität (Exponent von $\approx2$)\cite{Nesvorny2007}.
Dies stellten nur große Diskrepanzen zwischen Modell und Realität, jedoch schlug \cite{Nesvorny2007} mehrere Mechanismen vor, welche die Diskrepanz reduzieren könnte. 
% es wäre ein deutlich größeres Problem, wenn das Modell zu wenige Monde erzeugen würde. 
...
Damit stimmt die Gesamtanzahl und die SFD gut mit den Messwerten überein. % Bild?
Dabei hat diese Modell noch einen weiteren positiven Nebeneffekt. Die Zerstörung der irregulären Monde erzeugt eine große Menge Staub, der in der Zusammensetzung kohligen Chondriten ähnelt.
Ein Teil dieses Staubes würde durch nicht-gravitative Kräfte auf den Planeten zu driften und die Oberfläche regulärer Monde bedecken. Somit könnte dieses Modell auch eine Erklärung für die dunkle, nicht-eisige Oberfläche der äußeren regulären Monde von Jupiter, Saturn und Uranus erklären\cite{Bottke2010}.

Nachdem diese Probleme beseitigt werden konnten, bleibt die Frage der Interpretation der irregulären Monde von Saturn und vor allem Jupiter. In beiden Fällen sind selbst die maximalen Einfangseffizienzen kleiner als die durchschnittlichen Effizienzen bei Uranus und Neptun, auch Berücksichtigung von Kollisionen durch \cite{Bottke2010} änderte daran nicht sehr viel, die Eingangseffizienz der Eisriesen blieb 3 bis 10 mal höher als bei den Gasriesen.
...
%%%% x:y MMR oder x:y-MMR (u. x:y Resonanz) vereinheitlichen
