...
\end{equation}
wobei $a$ die große Halbachse ist. Formt man dies nach $a$ um, so erhält man:
\begin{equation}
a = \frac{G M r}{2 G M-r v^2}
\end{equation}
...
\begin{equation}
\Delta v = C \rho \frac{\left(GM\right)^2}{Mv^2} \Delta t
\end{equation}
Diese Abbremsung führt nun natürlich zu einer Bahnveränderung, die wir näherungsweise durch eine Veränderung der \glqq großen Halbachse\grqq\footnote{Hier in Anführungszeichen, da man genau genommen in diesem Fall keine Ellipse und somit auch keine große Halbachse mehr hat.} $a$ betrachten können:
\begin{equation}
\Delta a = \od{a}{v}\Delta v = \frac{2GMr^2v}{\left(rv^2-2GM\right)^2} \cdot \Delta v
\end{equation}
durch einsetzen erhalten wir
\begin{equation}
\Delta a = \frac{2r^2}{\left( rv^2-2GM\right)^2}\cdot C \rho G^3M^2 \Delta t \propto \frac{1}{v^4}
\end{equation}
Die Veränderung von $a$ ist also um so stärker, desto langsamer das Teilchen bewegt. Nachdem sich nun bekanntermaßen (und auch aus Gleichung \ref{eq:Bahngeschwindigkeit} ablesbar) der Planet um so schneller bewegt, je näher er an diesem ist, können wir daraus schließen, dass er an Sonnenfernen Punkten eine deutlich stärkere Bahnveränderung hin zur Sonne erfährt, als an Sonnennahen. Somit sinkt die Exzentrizität.

\subsection{Planetenentstehung} % weiter ausbauen
Das heutige Standardmodell der Stern- und Planetenentstehung geht vom Gravitationskollaps in einer großen molekularen Wolke aus, die auf 99\% Wasserstoff- und Heliumgas sowie etwa $0,1\mathrm{\mu m}$ große Staubteilchen besteht. Es entsteht ein Protostern in mitten der sich durch die Drehimpulserhaltung zunächst immer schneller drehenden und dadurch abgeplatteten Gasscheibe.
Später wird der Drehimpuls nach außen transportiert. % durch was
Durch die Brown'sche Bewegung kommt es zu zufälligen Zusammenstößen in der Gaswolke, wodurch die Teilchengrößen anwachsen. Diese wachsen durch akkretion von Gas und später durch Kollisionen weiter bis hin zu etwa einem Kilometer großen Klumpen, Planetesimale genannt.
...
Durch die Wechselwirkung des planetaren Embryos mit der Gasscheibe in die er noch immer eingebettet ist, kommt es zu einer Migration des Planeten nach innen. Als sich dann die Gasscheibe auflöst befinden sich die Planeten auf engen, kompakten, kreisförmigen Orbits, welche sich meist in Mean-Motion-Resonanzen befinden\cite{Nesvorny2011}.
