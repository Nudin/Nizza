...
Durch die Brown'sche Bewegung kommt es zu zufälligen Zusammenstößen in der Gaswolke, wodurch die Teilchengrößen anwachsen. Sie wachsen durch Akkretion von Gas und später durch Kollisionen weiter bis hin zu etliche Kilometer großen Klumpen, Planetesimale genannt.
...
Da sich die Planeten\footnote{Sofern nicht anderes angegeben, betrachten wir im folgenden immer nur die vier Gasriesen Jupiter, Saturn, Uranus und Neptun. Die terrestrischen Planten sind für diese Betrachtungen aufgrund der geringen Masse vernachlässigbar.} im heutigem Sonnensystem und bei anderen Planetensystemen auf weiter entfernten Bahnen befinden, braucht es einen Mechanismus, welcher die Planeten wieder auf weiter entfernte Bahnen bringt.
...
\end{equation}
Es ist natürlich auch Möglich die Schritte in der anderen Reihenfolge zu Kombinieren, also von $t=-\epsilon$ bis $t=h-\epsilon$ zu Integrieren, man erhält dann den gleichwertigen aber unterschiedlichen \textit{modifizierten kick-drift Euler Integrator}.
...
\end{equation}
wobei $\OA=\lbrace \Cdot, H_A \rbrace$ und $\OB=\lbrace \Cdot, H_B \rbrace$ nicht kommutierende Operatoren sind\cite{Chambers1999}. Betrachten wir die die Exponentialfunktion bis zum quadratischen Term
\begin{equation}
...
\begin{equation}
q(t+h)=e^{h\OB/2}e^{h\OA}e^{h\OB/2} \label{eq:secondorder}
\end{equation}
...
Wir betrachten die Massenpunkte $m_j$ und $m_k$, diese ersetzen wir durch einen virtuellen Massenpunkt $m_{jk}$ am Ort des Schwerpunktes der Massen $\vec R_{jk} = (m_j \vec q_j + m_k \vec q_k)/(m_j + m_k)$ und erhalten die Relativkoordinate $\vec x'_{1} = \vec x_j - \vec x_k $.
Wir iterieren diese Verfahren nun mit den restlichen $n-2$ Massenpunkten, sowie dem neuen virtuellen Massenpunkt. Am Ende haben wir $n-1$ relative Koordinaten $\vec x'_{i}$ sowie den zuletzt berechneten $\vec R_{1n}=:\vec x'_0$ welcher die Koordinate des Schwerpunkts des Systems angibt.
...
\end{equation}
ergibt. Dabei sind auch alle höheren Terme von A und B abhängig\cite{Chambers1999}. Für ein System für das gilt $\OB\sim\epsilon \OA$ mit kleinem Epsilon, so ist der Fehler auch proportional zu $\epsilon$.
...
Die Masse an Asteroiden die den Mond gemäß diesen Simulationen trifft wurde auf $\unit[\left( 3\text{--}8 \right) \cdot 10^{21}]{g}$, % space?
